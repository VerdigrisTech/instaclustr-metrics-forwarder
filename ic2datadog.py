...
cluster_id = os.environ['IC_CLUSTER_ID']

initialize(**dd_options)

auth_details = HTTPBasicAuth(username=ic_options['username'], password=ic_options['api_key'])

consecutive_fails = 0

while True:
    response = requests.get(url="https://api.instaclustr.com/monitoring/v1/clusters/{0}?metrics={1},".format(cluster_id, configuration['metrics_list']), auth=auth_details)
        ...
        for metric in node["payload"]:
            ...
            if metric["metric"] == "nodeStatus":
                ...
                if metric["values"][0]["value"] =="WARN":
                    statsd.service_check(dd_metric_name, 1, tags=configuration.get('tags', []) + tag_list) # WARN status
                else:
                    statsd.service_check(dd_metric_name, 0, tags=configuration.get('tags', []) + tag_list) # OK status
            else:
                # all other metrics map to a data dog guage
                statsd.gauge(dd_metric_name, metric["values"][0]["value"], tags=configuration.get('tags', []) + tag_list)
